...
public class TeamLeader extends Approver {
     ...
     * 团队领导的审批额度
     */
    private static final BigDecimal APPROVAL_LIMIT = new BigDecimal("1000");
    ...
    public void processRequest(ApprovalRequest request) {
        ...
        if (request.getAmount().compareTo(APPROVAL_LIMIT) <= 0) {
            ...
            logger.info("团队领导 " + name + " 审批通过了请求：" + request.getRequestId() + 
                    "，金额：" + request.getAmount() + "，类型：" + request.getRequestType());
        } else if (nextApprover != null) {
            // 超过审批额度，转交给下一个审批者
            logger.info("团队领导 " + name + " 无权审批请求：" + request.getRequestId() + 
